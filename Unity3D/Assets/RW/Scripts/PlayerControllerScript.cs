	...
	void Start () 
	{
		...
		for (int i=0; i<arrHeight; i++){
			for(int j=0; j<arrWidth; j++){
				...
				// 𝑥𝑃2=𝑥𝑃1+𝑟sin𝜃; 
				// 𝑦𝑃2=𝑦𝑃1−𝑟(1−cos𝜃)
				instanceCube.transform.position = new Vector3((float)(-19.5 + 6.3*Mathf.Sin((float)-j/arrWidth*360*Mathf.PI/180 + Mathf.PI/2)), (float)(20 - i), 
																(float)(-1.8 - 6.3*(1-Mathf.Cos((float)-j/arrWidth*360*Mathf.PI/180 + Mathf.PI/2))));

				instanceCube.transform.Rotate(0.0f, (float)(-(float)j/arrWidth*360 + 90), 0.0f, Space.World);
	...
	void Update () 
	{
		...
		for(int i=0; i<arrHeight; i++){
			for(int j=0; j<arrWidth/2; j++){
				...
				instanceCube = pixelArray[i, j+arrWidth/2];
				instanceCube.transform.localScale = cubeScale;
			}
		}
	}
